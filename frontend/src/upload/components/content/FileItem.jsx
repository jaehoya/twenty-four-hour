...
function FileItem({ item, isSelected = false, onClick, onFileDeleted, activeTab = 'storage', onSuggestionHandled }) {
    ...
    const handleCopyShareLink = async () => {
        ...
        try {
            ...
            if (response.status === 201 && response.data.data.shareLink) {
                await navigator.clipboard.writeText(response.data.data.shareLink);
            ...
            if (error.response?.data?.message) {
                alert(`링크 복사 실패: ${error.response.data.message}`);
            } else {
    ...
    return (
        <>
            ...
            <FileMenu
                ...
                onCopyLink={isTrashTab ? undefined : handleCopyShareLink}
