...
async function getTrashedFiles(userId) {
    ...
    // ⭐ 삭제된 폴더에 명시적 플래그 추가
    const deletedFoldersWithMeta = deletedFolders.map(f => ({
        ...f.toJSON(),
    ...
    return {
        folders: [
            ...deletedFoldersWithMeta,
            ...virtualFoldersWithMeta,
        ],
        files,
    };
}
...
async function restoreFolderRecursive(userId, folderId) {
    ...
    const subFolders = await Folder.findAll({
        where: { userId, parentId: folderId, deletedAt: {
                [Op.ne]: null } },
        paranoid: false,
    });
