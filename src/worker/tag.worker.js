...
const tagWorker = new Worker('aiTagQueue', async (job) => {
  ...
  // 3. 폴더 정리 제안 (바로 이동하지 않고 제안만 DB에 저장)
  try {
    const userFolders = await getAllFoldersFlat(file.user_id);
    const suggestedFolder = await recommendFolderForFile(recommendedTags, userFolders);
    ...
    console.error(`[WORKER] Folder suggestion failed:`, folderErr);
  }
